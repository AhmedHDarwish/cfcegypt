...
class AccountMoveLine(models.Model):
    ...
    def round_float(self,amount):
        coff =  -1 if self.move_id.move_type == 'out_refund' else 1
        return float("{:.2f}".format(coff * amount))
    ...
    def get_totals(self,lines):
        ...
        for line in lines:
            ...
            totals['order_qty'] = totals.get('order_qty',0) + coff * line.get_order_qty() 
            totals['delivery_total_qty'] = totals.get('delivery_qty',0) + coff * line.get_delivery_qty()
            totals['delivery_qty'] = totals.get('delivery_qty',0) + coff * line.quantity
            totals['invoice_value'] = totals.get('invoice_value',0) + coff * line.get_value_without_tax()
            totals['unit_price'] = totals.get('unit_price',0) + coff * line.price_unit
            cost = self.get_cost(line.product_id)
            totals['cost'] = totals.get('cost',0) + coff * cost
            totals['margin'] = totals.get('margin',0) + coff * line.get_margin()
            totals['total_cost'] = totals.get('total_cost',0) + coff * line.get_total_cost()
            totals['total_margin'] = totals.get('total_margin',0) + coff * line.get_total_margin()
